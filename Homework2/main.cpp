...
int main(int argc, char** argv) {
   ...
   if (argc < 2){
    ...
    cout << "Running NLM Filter..." << endl;
    
    cv::Mat_<float> nlmResult = dip2::nlmFilter(noisyImage[dip2::NOISE_TYPE_2], 21, 10.0);
    
    cv::imwrite("restorated__NOISE_TYPE_2__NLM_FILTER.jpg", nlmResult);
    
    cv::Mat_<float> diff_nlm = nlmResult - originalImage;
    float meanSqrDiff_nlm = cv::mean(diff_nlm.mul(diff_nlm))[0];
    float PSNR_nlm = 10.0f * std::log10(255*255 / meanSqrDiff_nlm);
    cout << "PSNR for NOISE_TYPE_2 with NLM_FILTER: " << PSNR_nlm << " dB" << endl;
    
    cout << "NLM Test Done." << endl;
