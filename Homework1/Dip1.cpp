...
cv::Mat doSomethingThatMyTutorIsGonnaLike(const cv::Mat& img) {
    ...
    cv::Mat rotatedImage;
    cv::warpAffine(result, rotatedImage, rotMatrix, result.size());
    ...
    cv::Mat blurredImage;
    cv::GaussianBlur(histImage, blurredImage, cv::Size(9, 9), 0);

    cv::subtract(cv::Scalar(255), blurredImage, result);
